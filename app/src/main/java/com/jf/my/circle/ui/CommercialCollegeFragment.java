...
import com.jf.my.Module.common.Dialog.HintDialog;
...
import com.jf.my.utils.UIUtils;
...
//        });
        ll_information_more.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                MiYuanInformationFragment.start(getActivity(),mMiyuanInformationList);
            }
        });
...
//        mReUseListView.getListView().refreshComplete(REQUEST_COUNT);
    ...
    @Override
    public void onCarouselSuc(List<MiyuanInformation> data) {
        mMiyuanInformationList.clear();
        mMiyuanInformationList.addAll(data);
        setHotView(data);
    }
    ...
    private void setHotView(final List<MiyuanInformation> datas) {
        //蜜源热门
        if (datas == null || datas.size() == 0) {
            return;
        }
