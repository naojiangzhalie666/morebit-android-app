...
import com.jf.my.utils.CopyPropertiesUtil;
import com.jf.my.utils.DateTimeUtils;
import com.jf.my.utils.DensityUtil;
import com.jf.my.utils.DownloadManage;
...
/**
 */
public class MiYuanCircleAdapter extends BaseMYShareAdapter<MiYuanCircleAdapter.CommonViewHolder> {
    ...
    private List<MiYuanCircleInfo> mDataList = new ArrayList<>();
    ...
    private MyAction.Two<MiYuanCircleInfo,Integer> mbrandCollectAction;
    CircleBrandsAdapter circleBrandsAdapter;
    public MiYuanCircleAdapter(Context context, int loadType) {
        super();
...
//        mTasks = new ArrayList<>();
    ...
    public void updateItem(MiYuanCircleInfo item,int pos){
        if(null != mDataList){
            Iterator<MiYuanCircleInfo> iterator = mDataList.iterator();
            for (int i = 0; i < mDataList.size(); i++) {
                MiYuanCircleInfo eachItem =  mDataList.get(i);
    ...
    public void updateBrandItem(MiYuanCircleInfo item,int pos){
        if(null != circleBrandsAdapter){
           List<MiYuanCircleInfo> items =  item.getCircleBrands();
    ...
    public void removeItem(MiYuanCircleInfo item,int pos){
        if(null != mDataList){
            int cur = 0;
            Iterator<MiYuanCircleInfo> iterator = mDataList.iterator();
            while (iterator.hasNext()){
                MiYuanCircleInfo eachItem = iterator.next();
                if(eachItem.getId() == item.getId()){
                    iterator.remove();
    ...
    public void insertItem(MiYuanCircleInfo item,int pos){
        if(null != mDataList){
    ...
    public class BrandViewHolder extends CommonViewHolder{
        ...
        public void bind(final BrandViewHolder holder, final int position, final MiYuanCircleInfo item) {
            if(null != item && item.isShowCircleBrand() && null != item.getCircleBrands() && item.getCircleBrands().size()>0){
                ...
                circleBrandsAdapter.setmCollectAction(new MyAction.Two<MiYuanCircleInfo, Integer>() {
                    @Override
                    public void invoke(MiYuanCircleInfo childItem, Integer position) {
    ...
    public class PostViewHolder extends CommonViewHolder {
        ...
        MiYuanCirleItmeAdapter cirleDayHotItmeAdapter;
        MiYuanCirleItmeGoodsAdapter cirleDayHotItmeGoodsAdapter;
        ...
        public void bind(final PostViewHolder holder, final int position, final MiYuanCircleInfo item) {
...
//                        }
            ...
            if (holder.cirleDayHotItmeAdapter == null) {
                ...
                holder.cirleDayHotItmeAdapter = new MiYuanCirleItmeAdapter(mContext, item, mLoadType, imgWidth, isOneImg, mWindowWidth3);
            ...
            if (holder.cirleDayHotItmeGoodsAdapter == null) {
                holder.cirleDayHotItmeGoodsAdapter = new MiYuanCirleItmeGoodsAdapter(mContext, item);
        ...
        private void setGridManager(final boolean isOneImg, GridLayoutManager layoutManager) {
            layoutManager.setSpanSizeLookup(new GridLayoutManager.SpanSizeLookup() {
                ...
                public int getSpanSize(int position) {
                    MyLog.d("MiYuanCirleItmeAdapter", "isOneImg " + isOneImg + "  position  " + position);
                    if (isOneImg) {
                        return 3;
                    } else {
                        return 1;
    ...
    private void share(final MiYuanCircleInfo item, int type, int sharePlatform) {
        switch (type) {
...
//                            }
                    ...
                    //过滤视频图片
                    for (MiyuanCircleItemInfo miyuanCircleItemInfo : item.getShareRangItems()) {
                        if (StringsUtils.isEmpty(miyuanCircleItemInfo.getItemVideoid()) && miyuanCircleItemInfo.getIsExpire() == 0) {
                            pictures.add(miyuanCircleItemInfo.getPicture());
                        }
...
//                List<ShopGoodInfo> data = GoodsUtil.CirleInfoToGoodInfo(item);
    ...
    private List<ShopGoodInfo> addGoods(MiYuanCircleInfo item) {
        List<ShopGoodInfo> goodsList = new ArrayList<>();
        List<MiyuanCircleItemInfo> miyuanList = item.getShareRangItems();
        if (miyuanList != null) {
            for (int i = 0; i < miyuanList.size(); i++) {
                MiyuanCircleItemInfo miyuanCircleItemInfo = miyuanList.get(i);
                //只有商品分享才能分享二维码海报
                if (StringsUtils.isEmpty(miyuanCircleItemInfo.getItemVideoid()) && !TextUtils.isEmpty(miyuanCircleItemInfo.getItemSourceId())) {
                    ShopGoodInfo shopGood = new ShopGoodInfo();
                    try {
                        CopyPropertiesUtil.copyProperties(miyuanList.get(i), shopGood);
...
//        mContext.startActivity(intent);
     ...
     * 分享dialog
     */
    protected void showChoosePicDialog(final Activity activity, final MiYuanCircleInfo item) {
...
//                    openShareDialog(item,CircleDayHotFragment.TYPE_SHARE_VIDEO);
    ...
    private int getShareDialogType(MiYuanCircleInfo item) {
        ...
        if (item.getShareRangItems() != null) {
            for (MiyuanCircleItemInfo miyuanCircleItemInfo : item.getShareRangItems()) {
                if (!StringsUtils.isEmpty(miyuanCircleItemInfo.getItemVideoid())) {
                    isExitVideo = true;
                    continue;
    ...
    private void openShareDialog(final MiYuanCircleInfo item, final int type) {
    ...
    private void download(MiYuanCircleInfo item, int sharePlatform) {
        if (item == null) {
            return;
        }
        ...
        if (item.getShareRangItems() != null) {
            ...
            for (int i = 0; i < item.getShareRangItems().size(); i++) {
                MiyuanCircleItemInfo circleItemInfo = item.getShareRangItems().get(i);
...
//                       mTasks.add(FileDownloader.getImpl().create(videoUrl).setPath(SdDirPath.IMAGE_CACHE_PATH+fileName+"."+suffix).setTag(i + 1));
    ...
    public void setMbrandCollectAction(MyAction.Two<MiYuanCircleInfo, Integer> mbrandCollectAction) {
