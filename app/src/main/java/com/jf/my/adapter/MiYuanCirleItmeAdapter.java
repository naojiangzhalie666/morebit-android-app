...
public class MiYuanCirleItmeAdapter extends RecyclerView.Adapter {
    ...
    private MiYuanCircleInfo mDatas;
    private int mImgWidth;
    private int mWindowWidth;
    private String mMainTitle,mSubTitle;
    public MiYuanCirleItmeAdapter(Context context, MiYuanCircleInfo images, int loadType, int imgWidth, boolean isOneImg, int windowWidth) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        ...
        if (mDatas.getShareRangItems() != null && mDatas.getShareRangItems().size() > 0) {
            final MiyuanCircleItemInfo circleItemInfo = mDatas.getShareRangItems().get(position);
...
//                        SensorsDataUtil.getInstance().mifenClickTrack(mMainTitle,mSubTitle,"蜜粉圈",position,circleItemInfo.getItemSourceId(),circleItemInfo.getItemTitle(),"","",circleItemInfo.getPicture(),circleItemInfo.getItemTitle(),"",0,"");
                        if (!StringsUtils.isEmpty(circleItemInfo.getItemVideoid())) {  //视频
                            ...
                            List<MiyuanCircleItemInfo> miyuanCircleItemInfoList =mDatas.getShareRangItems();
                            List<String> pictures = new ArrayList<String>();
                            for(MiyuanCircleItemInfo miyuanCircleItemInfo:miyuanCircleItemInfoList){
                                if(StringsUtils.isEmpty(miyuanCircleItemInfo.getItemVideoid())&&StringsUtils.isEmpty(miyuanCircleItemInfo.getItemSourceId())&&!TextUtils.isEmpty(miyuanCircleItemInfo.getPicture())){
    ...
    public void seData(MiYuanCircleInfo item) {
        mDatas = item;
