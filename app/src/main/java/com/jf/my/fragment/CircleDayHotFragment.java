...
public class CircleDayHotFragment extends BaseFragment {
    ...
    public void initView(View view) {
        ...
        mAdapter = new MiYuanCircleAdapter(getActivity(), mLoadType);
        ...
        mAdapter.setmCollectAction(new MyAction.Two<MiYuanCircleInfo, Integer>() {
            @Override
            public void invoke(MiYuanCircleInfo item, Integer position) {
        ...
        mAdapter.setMbrandCollectAction(new MyAction.Two<MiYuanCircleInfo, Integer>() {
            @Override
            public void invoke(MiYuanCircleInfo item, Integer position) {
                MiYuanCircleInfo childInfo =  item.getCircleBrands().get(position);
    ...
    public void getFirstData() {
        ...
        Observable<BaseResponse<List<MiYuanCircleInfo>>> compose = getObservable();
        compose.doFinally(new Action() {
            ...
            public void run() throws Exception {
                ...
                .subscribe(new DataObserver<List<MiYuanCircleInfo>>(false) {
                    @Override
                    ...
                    @Override
                    public void onSuccess(List<MiYuanCircleInfo> agentDetailList) {
    ...
    public void getMoreData() {
        Observable<BaseResponse<List<MiYuanCircleInfo>>> compose = getObservable();
        compose.subscribe(new DataObserver<List<MiYuanCircleInfo>>() {
            @Override
    ...
    private Observable<BaseResponse<List<MiYuanCircleInfo>>> getObservable() {
        RequestMiYuanCircleBean requestBean = new RequestMiYuanCircleBean();
        requestBean.setPage(pageNum);
        if (mLoadType == TypeThisMaterial) {
            return RxHttp.getInstance().getSysteService()
                    .getMaterial(requestBean)
                    .compose(RxUtils.<BaseResponse<List<MiYuanCircleInfo>>>switchSchedulers())
                    .compose(this.<BaseResponse<List<MiYuanCircleInfo>>>bindToLifecycle());
     ...
     */
    private void addCollect(final MiYuanCircleInfo item, final int itemPosition,final boolean isChild) {
        int requestId = 0;
        if(!isChild){
                    ...
                    if (null != mAdapter) {
                        ...
                        if(!isChild){
                            ...
                            if(null != item.getCircleBrands() && item.getCircleBrands().size()>0){
                                List<MiYuanCircleInfo> list = item.getCircleBrands();
                                MiYuanCircleInfo info =  list.get(itemPosition);
    ...
    private void removeCollect(final MiYuanCircleInfo item, final int itemPosition,final boolean isChild) {
        String collectId = "";
        String rangId = "";
        if(!isChild){
        ...
        compose.subscribe(new DataObserver<List<CircleBrand>>() {
            ...
            protected void onSuccess(List<CircleBrand> datas) {
                if (null != datas && datas.size() > 0) {
                    ...
                    miYuanCircleInfo.setShowCircleBrand(true);
                    miYuanCircleInfo.setCircleBrands(MyGsonUtils.toMiYuanCircleInfo(datas));
//                    if(null != listArray && listArray.size() >1){
//
//                       List<MiYuanCircleInfo> showDatas =  listArray.get(1).getCircleBrands();
...
//                        }
//                    }
                    listArray.add(1, miYuanCircleInfo);
                    mAdapter.insertItem(miYuanCircleInfo, 1);
                }
...
//                        SensorsDataUtil.getInstance().advClickTrack(imageInfo.getId()+"",level,level2,imageInfo.getOpen()+"","蜜粉圈",position,imageInfo.getTitle(),imageInfo.getClassId()+"",imageInfo.getUrl(),"");
     ...
     * 局部刷新列表
     */
    private void refreshList(MiYuanCircleInfo updateInfo){
        boolean isfind = false;
        int  childPos = 0;
        if (mAdapter != null) {
            if(null != mAdapter && mAdapter.getDatas().size()> 1){
                for (int i = 0; i < 2 ; i++) {
                    MiYuanCircleInfo eachItem =  mAdapter.getDatas().get(i);
                    List<MiYuanCircleInfo> childItems =  eachItem.getCircleBrands();
                    if(null != childItems && childItems.size()>0){
                        for (int j = 0; j < childItems.size(); j++) {
                            MiYuanCircleInfo eachChild =  childItems.get(j);
