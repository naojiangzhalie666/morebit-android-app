 ...
 * @Description: 二级模块
 **/
public class ActivityFloorAdapter extends RecyclerView.Adapter<ActivityFloorAdapter.ViewHolder>{
    public static final int DISPLAY_HORIZONTAL = 0; //横向
    private LayoutInflater mInflater;
    private List<ImageInfo> mDatas = new ArrayList<>();
    private Context mContext;

    public ActivityFloorAdapter(Context context, List<ImageInfo> data) {
    ...
    @Override
    public void onBindViewHolder(ActivityFloorAdapter.ViewHolder holder, final int position) {
        ...
        if (!TextUtils.isEmpty(imageInfo.getPicture()) && imageInfo.getId()!=-1) {
            ...
            LoadImgUtils.loadingCornerTop(mContext, viewHolder.imageView, imageInfo.getPicture(),8);
        }else{
