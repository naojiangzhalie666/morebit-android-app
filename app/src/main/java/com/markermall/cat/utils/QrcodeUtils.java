...
public class QrcodeUtils {
    ...
    public static Bitmap createQRCode(String text, int size) {
        try {
            ...
            hints.put(EncodeHintType.MARGIN, 0);//此处设置边框大小（0：无边框）
              /*
             * 设置容错级别，默认为ErrorCorrectionLevel.L
             * 因为中间加入logo所以建议您把容错级别调至H,否则可能会出现识别不了
             */
...
//            Log.d("miuanapp  ","EncodeHintType  "+ErrorCorrectionLevel.M.toString());
//            Log.d("miuanapp  ","text  "+text);
//            Log.d("miuanapp  ","size  "+size);
//            Log.d("miuanapp  ","hints  "+hints);
            BitMatrix bitMatrix = new QRCodeWriter().encode(text, BarcodeFormat.QR_CODE, size, size, hints);
//            Log.d("miuanapp  ","bitMatrix  "+bitMatrix.toString());
