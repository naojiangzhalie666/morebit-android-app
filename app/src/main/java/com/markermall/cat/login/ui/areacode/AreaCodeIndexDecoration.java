package com.markermall.cat.login.ui.areacode;
...
import com.markermall.cat.pojo.AreaDataItem;
...
public class AreaCodeIndexDecoration extends RecyclerView.ItemDecoration {
    ...
    public AreaCodeIndexDecoration(int indexViewHeight, int backgroundColor, int textSize, int textColor) {
    ...
    public void getItemOffsets(@NonNull Rect outRect, @NonNull View view, @NonNull RecyclerView parent, @NonNull RecyclerView.State state) {
        AreaDataItem item = (AreaDataItem) view.getTag();
        if (item.isHead()) {
    ...
    public void onDrawOver(@NonNull Canvas c, @NonNull RecyclerView parent, @NonNull RecyclerView.State state) {
        ...
        AreaDataItem firstItem = (AreaDataItem) firstVisibleView.getTag();
        ...
        if (null == secondVisibleViewHolder) {
            ...
            secondViewIsTop = ((AreaDataItem) secondVisibleItemView.getTag()).isHead();
        }
    ...
    public void onDraw(@NonNull Canvas c, @NonNull RecyclerView parent, @NonNull RecyclerView.State state) {
        ...
        for (int i = 0; i < childCount; i++) {
            AreaDataItem item = (AreaDataItem) parent.getChildAt(i).getTag();
            if (item.isHead()) {
    ...
    private void drawIndexView(Canvas canvas, String indexStr, Rect rect) {
        ...
        canvas.drawText(indexStr, 40, groupTextY, mTextPaint);
    }
