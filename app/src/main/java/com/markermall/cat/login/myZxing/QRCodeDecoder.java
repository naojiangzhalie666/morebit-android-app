...
public class QRCodeDecoder {
    ...
    static {
        ...
        ALL_HINT_MAP.put(DecodeHintType.ALLOWED_EAN_EXTENSIONS, Boolean.TRUE);
        // 编码字符集
    ...
    public static String syncDecodeQRCode(Bitmap bitmap) {
        ...
        try {
            ...
            result = new MultiFormatReader().decode(new BinaryBitmap(new HybridBinarizer(source)), QR_CODE_HINT_MAP);
            return result.getText();
        } catch (Exception e) {
            e.printStackTrace();
            if (source != null) {
                try {
                    result = new MultiFormatReader().decode(new BinaryBitmap(new GlobalHistogramBinarizer(source)), QR_CODE_HINT_MAP);
                    return result.getText();
                } catch (Throwable e2) {
                    e2.printStackTrace();
                }
